...
export const AIInsightPanel: React.FC<AIInsightPanelProps> = ({ analysis }) => {
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <span className="relative flex h-2 w-2">
              <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${analysis ? 'bg-blue-400' : 'bg-gray-400'} opacity-75`}></span>
              <span className={`relative inline-flex rounded-full h-2 w-2 ${analysis ? 'bg-blue-500' : 'bg-gray-500'}`}></span>
            </span>
            <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
              {analysis ? 'AI Analysis Available' : 'No Recent Analysis'}
            </span>
          </div>
          <h2 className="text-xl md:text-2xl font-bold text-white leading-tight">
            {analysis ? 'Probable Cause Identified' : 'Awaiting Incident Detection'}
          </h2>
        </div>
        {analysis && (
